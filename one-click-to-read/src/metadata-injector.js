 ...
 *
 * API:
 *   window.WakaMetaInjector.injectIntoBlob(blob, titleOverride?, meta?) → Promise<Blob>
 *   window.WakaMetaInjector.setMeta(meta)         → void
 ...
 *
 * [userscript] Bản extension lưu metadata tạm qua chrome.runtime (background.js →
 * chrome.storage). Ở đây metadata giữ trong bộ nhớ của trang (một lượt "Đọc ngay" tại một
 * thời điểm), và ảnh bìa tải bằng WakaGM.fetch (GM_xmlhttpRequest, không bị CORS).
 */
const WakaMetaInjector = (() => {
  'use strict';

  // ── Storage (bộ nhớ trang; thay cho chrome.storage.local của extension) ─────
  let currentMeta = null;
  function setMeta(meta) { currentMeta = meta || null; }
  async function getMeta() { return currentMeta; }
  async function hasMeta() {
    const m = await getMeta();
    return !!(m && m.title);
  }
  async function clearMeta() { currentMeta = null; }
  ...
  // các trường khác (tác giả, ảnh bìa, mô tả...) vẫn lấy từ metadata đã lưu chung.
  async function injectIntoBlob(epubBlob, titleOverride, metaArg) {
    const meta = metaArg || await getMeta();
    if (!meta || !meta.title) {
  ...
  return { injectIntoBlob, hasMeta, getMeta, setMeta, clearMeta };
})();
