...
const WakaEpubDecode = (() => {
  ...
  function cleanTitle(s) {
    return String(s || '')
      // Bỏ tiền tố "Đọc sách - ", "Đọc sách:", "Đọc_sách_-_" ...
      .replace(/^Đọc[\s_]*sách[\s_]*[-–:_]*[\s_]*/i, '')
      // Bỏ hậu tố Waka / thư viện
      .replace(/\s*[-–]\s*.*Waka.*$/i, '')
  ...
  function extractTitleFromOpf(opfText, fallbackTitle = 'waka-ebook') {
    ...
    if (!titleMatch) return cleanTitle(fallbackTitle) || 'waka-ebook';
    ...
    return cleanTitle(title) || cleanTitle(fallbackTitle) || 'waka-ebook';
  }
