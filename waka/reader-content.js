...
(function () {
  ...
  window.addEventListener('__waka_epub_found__', (e) => {
    ...
    let t = e.detail.title || 'Ebook';
    if (window.WakaEpubDecode?.cleanTitle) t = window.WakaEpubDecode.cleanTitle(t);
    else {
      t = String(t)
  ...
  async function buildAndDownload() {
    try {
      ...
      if (window.WakaMetaInjector) {
        ...
        if (hasMeta) {
          setStatus('📚 Đang nhúng metadata + ảnh bìa vào EPUB...');
          try {
    ...
    if (hasMeta) {
      ...
      if (!badge) {
        ...
        badge.style.cssText = [
          ...
          'border:1px solid #1a5c33','cursor:pointer',
        ].join(';');
        ...
        const root = document.getElementById('wdl-root');
        if (root) root.insertBefore(badge, root.firstChild);
      }
